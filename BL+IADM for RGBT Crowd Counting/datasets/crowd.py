...
class Crowd(data.Dataset):
    def __init__(self, root_path, crop_size=256,
        ...
        self.gt_list = sorted(glob(os.path.join(self.root_path, 'labels/*.npy')))  # change to npy for gt_list
        print(os.path.join(self.root_path, '*.npy'))
        print(f'Loaded {len(self.gt_list)} ground truth files for {method}.')
    ...
    def __getitem__(self, item):
        ...
        rgb_path = gt_path.replace('labels', 'rgb').replace('R.npy', '.jpg')
        t_path = gt_path.replace('labels', 'tir').replace('npy', 'jpg')
